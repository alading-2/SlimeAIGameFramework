...
> 日期：2026-05-13
> 状态：架构探索记录，不是已实施契约。
> 主题：判断 SkilmeAI 当前框架是否仍应称为 ECS，组合式解耦是否真的帮助 AI，以及哪些成熟引擎机制适合转译进 AI-first GameOS。
> 输入：SkilmeAI 当前 `DocsAI`、`GameOS` 源码、本地 `Engine/Docs/FrameworkAnalysis/Reports/01-11` 与 `99` 综合报告、Bevy / Unity Entities / Unreal / Godot / Component Pattern / AI coding agent 官方资料复核。

## 0. 最终判断

**SkilmeAI 不应再对外称为传统 ECS 框架。**
...
用户说的“组合优于继承”判断成立，但它只解释了一部分。SkilmeAI 当前更核心的思想是：
...
SkilmeAI 当前不是这个模型。它没有把 gameplay 主入口建成 archetype / sparse-set / chunk / global query DSL；`Component` 在 GodotBridge 边界也不是纯数据，允许有生命周期逻辑；大多数玩法逻辑在 Capability service、Tool、Handler 中。
...
AI 不擅长无限自由的运行时拼装。如果允许任何组件在任何时间、任何系统遍历中随意挂载、删除、隐式改关系，AI 会更难推理时序、所有权和验证证据。因此 SkilmeAI 要保留“高自由组合”的方向，但必须把自由收束成：
...
```text
SkilmeAI 是 AI-first GameOS 框架，不是通用游戏引擎。
...
因此 Data 不应退回裸 string，也不需要改成传统 ECS component storage。DataKey 是 SkilmeAI 的状态契约核心。

### 2.4 Component 在 SkilmeAI 中不是传统纯数据组件
...
1. 本地源码/报告：`Engine/Docs/FrameworkAnalysis/Reports/01-11` 与 `99-SkilmeAI-引擎源码综合分析报告.md`。
...
对 SkilmeAI 的启示不是“复制 Bevy ECS”，而是：
...
- 不新增 AttributeSet 作为第二套数值状态源；SkilmeAI 已有 DataKey。
...
但 SkilmeAI 不能把 Godot Node tree 当 gameplay source-of-truth。原因：
...
- archetype graph、wildcard query、pair traversal、25+ 泛型 query 重载等不适合成为 SkilmeAI AI-facing API。
...
这从 AI 工程侧支持 SkilmeAI 当前 DocsAI / skill / owner capability / validation artifact 路线。也说明“AI 框架”不能只依赖 runtime 设计，还要把 AI 的工作流做成框架的一部分。
...
SkilmeAI 当前模式如果继续收紧，可以逐个压低这些风险：
...
- 在 SkilmeAI 中 Godot component 有生命周期和逻辑。
...
- 对外定位不要写“SkilmeAI 是 ECS 框架”。
- AI 入口文档里加一句：`SkilmeAI is not a traditional ECS; do not add a global world query or pure ECS component storage unless an OpenSpec explicitly asks for it.`

## 6. 更好的核心模型：Capability Composition Runtime

建议把 SkilmeAI 的运行时心智模型固定成：
...
### 7.3 Reject：不建议进入 SkilmeAI 主 API
...
**SkilmeAI 每个单独机制都能在成熟引擎或 AI agent workflow 中找到先例；但把这些机制组合成面向 AI 执行、验证和复盘的 Godot C# GameOS，目前在本地调研范围内没有看到同构框架。**
...
这就是为什么 SkilmeAI 不应为了证明自己是 ECS 而回到 ECS，也不应为了显得“新”而拒绝成熟引擎机制。
...
```text
SkilmeAI 借鉴 ECS 和 Component pattern，但不是传统 ECS 框架。
...
- `Engine/Docs/FrameworkAnalysis/Reports/99-SkilmeAI-引擎源码综合分析报告.md`
- `Engine/Docs/FrameworkAnalysis/Reports/01-Bevy-源码分析报告.md`
- `Engine/Docs/FrameworkAnalysis/Reports/07-Unity-Entities-Samples-源码分析报告.md`
- `Engine/Docs/FrameworkAnalysis/Reports/09-Unreal-GAS-文档对照报告.md`
