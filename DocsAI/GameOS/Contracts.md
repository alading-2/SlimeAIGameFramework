...
- Framework Runtime/Events 只持有跨游戏 profile 有意义、且 payload 不直接依赖 Godot 引擎类型的事件。带具体游戏玩法术语或 Godot `Vector2 / Rect2 / Node` 等字段的事件必须放在 `Games/<Game>/Src/Game/Event/`；完整决策树见 `Workspace/DocsAI/AgentWorkflow/Protocols/FrameworkVsGameBoundary.md#事件归属决策树`。
