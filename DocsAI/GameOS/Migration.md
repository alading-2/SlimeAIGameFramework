...
- Runtime Entity 最小内核：
  ...
  - `RuntimeEntity`
  - `EntitySpawnConfig`（typed `EntityId / ParentEntityId`）
  - `EntityManager`（typed `Spawn / Get / Destroy / BindParentRelationships`，内部 `Dictionary<EntityId, IEntity>`）
...
- 受影响公开 API：`IEntity.EntityId`、`RuntimeEntity` 构造、`EntitySpawnConfig.EntityId / ParentEntityId`、`EntityManager.Spawn / Register / Destroy / Get / GetAll / Clear / BindParentRelationships`、`Runtime.Events.Core.RelationshipAdded / RelationshipRemoved` payload、`GameOSGodotBridge.GetEntityNode`。
...
- `RelationshipManager` 内部 API 仍以 string 为 entity-id（P1 范畴未动）；调用方使用 `entityId.Value` 适配。
